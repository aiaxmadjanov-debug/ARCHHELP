...
def load_users():
    ...
    try:
        ...
        print(f"❌ USERS.JSON OвЂQISH XATOSI: {e}")
        return {}
...
def save_users(users):
    try:
        ...
        print(f"❌ USERS.JSON SAQLASH XATOSI: {e}")
        ...
        if isinstance(event, Message):
            ...
            if not subscribed:
                await event.answer(
                    ...
                    "kanalimizga a’zo boвЂling.\n\n"
                    "📢 Kanal: @autocadrvt\n\n"
                    "1️⃣ Kanalga a’zo boвЂling\n"
                    "2️⃣ «✅ TekshirishВ» tugmasini bosing.",
...
AUTOCAD_TIPS = {
    ...
    "PURGE bilan tozalash": {
        "tushuntirish":
            "PURGE ishlatilmayotgan Layer, Block, Linetype va boshqa ma’lumotlarni tozalaydi.",
        "qadamlar": [
    ...
    "AutoSave va Recovery": {
        "tushuntirish":
            "AutoSave va Recovery ma’lumot yoвЂqolishining oldini olishga yordam beradi.",
        "qadamlar": [
            "OPTIONS ni oching.",
...
video_menu = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(
                text="💻 AutoCAD o‘rnatish",
                url="https://t.me/autocadrvt/204"
            )
        ],
        [
            InlineKeyboardButton(
                text="🏗 Revit o‘rnatish",
...
@dp.message(F.text == "📚 Videodarsliklar")
async def video_lessons_handler(message: Message):

    await message.answer(
        "📚 VIDEODARSLIKLAR\n\n"
        "Kerakli dastur bo‘yicha o‘rnatish "
...
async def start_handler(
    ...
    if not subscribed:
        await message.answer(
            "🔒 ARCHHELP BOT\n\n"
            "🏗 ARCHHELP'dan foydalanish uchun "
...
async def check_subscription_callback(
    ...
    if not subscribed:
        ...
        await callback.answer(
            "❌ Siz hali kanalga a'zo boвЂlmagansiz!",
            show_alert=True
        )
    ...
    if callback.message:
        ...
        try:
            await callback.message.edit_text(
                "✅ Kanalga a'zoligingiz tasdiqlandi!\n\n"
                "🎉 Endi ARCHHELP botidan foydalanishingiz mumkin."
            )
        except Exception:
            pass
...
@dp.message(F.text == "❌ Revit xatolari")
...
async def revit_error_detail(message: Message):
    ...
    # =====================================================
    # 🎥 YOUTUBE — XATONI AVTOMATIK QIDIRISH
    # =====================================================

    youtube_query = (
        f"Autodesk Revit {name} error fix solution tutorial"
    )
    ...
    youtube_query = f"Autodesk Revit {name} error fix solution tutorial"
    ...
    text = (
        ...
        f"💡 Maslahat:\n{data.get('maslahat', '-')}\n\n"
        f"🎥 Video darslik:\n"
        f'<a href="{youtube_url}">▶️ YouTube\'da yechimni ko‘rish</a>'
    )
...
@dp.message(F.text == "⚙️ Revit o‘rnatish")
async def revit_installation_handler(message: Message):

    buttons = [
        [KeyboardButton(text=f"⚙️ {name}")]
    ...
    await message.answer(
        f"⚙️ <b>Revit o‘rnatish va sozlash</b>\n\n"
        f"Jami: <b>{len(REVIT_INSTALLATION)} ta muammo</b>\n\n"
        f"Muammoni tanlang:",
        reply_markup=keyboard,
        parse_mode="HTML"
    )
...
async def revit_installation_detail(message: Message):
    ...
    text = (
        ...
        f"💡 <b>Maslahat:</b>\n{data.get('maslahat', '-')}\n\n"
        f"🎥 <b>Videodarslik:</b>\n"
        f'<a href="https://t.me/autocadrvt/3">▶️ Revit o‘rnatish videosini ko‘rish</a>'
    )
...
@dp.message(F.text == "💡 Revit maslahatlari")
...
@dp.message(F.text == "🧩 Revit Families")
...
@dp.message(F.text == "📏 Revit o‘lchamlar")
async def revit_dimensions_handler(message: Message):
    ...
    if not categories:
        await message.answer(
            "❌ Revit o‘lchamlar bazasida ma'lumot topilmadi."
        )
        return
    ...
    for category in categories:
        buttons.append([
            KeyboardButton(text=f"📐 {category}")
        ])
...
async def revit_dimension_detail(message: Message):
    ...
    band = data.get("band", "-")
    izoh = data.get("izoh", data.get("maslahat", "-"))
...
# AUTOCAD BUYRUQLARI
# =========================================================
@dp.message(F.text == "🛠️ AutoCAD buyruqlari")
async def autocad_commands_handler(message: Message):
    ...
    await message.answer(
        f"🛠️ <b>AutoCAD buyruqlari</b>\n\n"
        f"📊 Jami: <b>{len(AUTOCAD_COMMANDS)} ta</b>\n\n"
        "Kerakli buyruqni tanlang 👇",
        reply_markup=keyboard,
        parse_mode="HTML"
    )
# =========================================================
# AUTOCAD BUYRUQ TAFSILOTI
# =========================================================
@dp.message(
    lambda message: (
...
async def autocad_command_detail(message: Message):
    ...
    # 🔹 belgidan keyingi buyruq nomi
    name = message.text[2:].strip()

    # Buyruq bazada mavjudligini tekshirish
    if name not in AUTOCAD_COMMANDS:
        await message.answer(
            f"❌ <b>Buyruq topilmadi:</b>\n\n"
            f"<code>{name}</code>\n\n"
            f"📌 Bazada bunday AutoCAD buyrug‘i mavjud emas.",
            parse_mode="HTML"
        )
        return
...
async def revit_errors_handler(message: Message):
    ...
    await message.answer(
        f"❌ <b>Revit xatolari</b>\n\n"
        f"📊 Jami: <b>{len(REVIT_ERRORS)} ta</b>\n\n"
        "Muammoga mos xatoni tanlang 👇",
        reply_markup=keyboard,
        parse_mode="HTML"
    )
...
@dp.message(F.text == "💡 AutoCAD maslahatlari")
...
@dp.message(F.text == "📐 AutoCAD sozlamalari")
...
# =========================================================
# HISOBLAGICH — ASOSIY MENYU
# =========================================================

@dp.message(F.text == "📐 Hisoblagich")
...
async def brick_calculator_handler(
    ...
    await message.answer(
        "🧱 G‘ISHT KALKULYATORI\n\n"
        "1️⃣ Devor uzunligini kiriting.\n\n"
        "Masalan:\n"
        "10 m\n"
        "yoki\n"
        "10000 mm\n\n"
        "📌 m, cm yoki mm ishlatishingiz mumkin."
    )


# =========================================================
# O‘LCHOVNI METRGA O‘TKAZISH
# =========================================================
...
async def brick_length_handler(
    ...
    length = parse_measurement(message.text)

    if length is None or length <= 0:

        await message.answer(
            "❌ Noto‘g‘ri o‘lcham kiritildi.\n\n"
            "Masalan:\n"
            "10 m\n"
            "1000 cm\n"
            "10000 mm"
    ...
    await message.answer(
        "2️⃣ Devor balandligini kiriting.\n\n"
        "Masalan:\n"
        "3 m\n"
        "300 cm\n"
        "3000 mm"
...
async def brick_height_handler(
    ...
    if not message.text:
        ...
        await message.answer(
            "❌ Iltimos, devor balandligini kiriting."
        )
    ...
    if height <= 0:
        ...
        await message.answer(
            "❌ Balandlik 0 dan katta boвЂlishi kerak."
        )
    ...
    opening_menu = ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text="🚪 Eshik")
            ],
            [
                KeyboardButton(text="🪟 Deraza")
            ],
            [
                KeyboardButton(
                    text="🚪🪟 Eshik + deraza"
    ...
    await message.answer(
        f"✅ Devor balandligi: {height:g} m\n\n"
        "3️⃣ Devorning ichida eshik yoki derazalar bormi?\n\n"
        "📌 Eshik va derazalar maydoni "
...
async def brick_door_count_handler(
    ...
    if count <= 0:
        ...
        await message.answer(
            "❌ Eshik soni 0 dan katta boвЂlishi kerak."
        )
...
async def brick_door_length_handler(
    ...
    await message.answer(
        f"✅ Eshik uzunligi: {length:g} m\n\n"
        "🚪 Endi eshik boвЂyini kiriting.\n\n"
        "Masalan:\n"
        "2100 mm\n"
        "yoki\n"
        "2.1 m"
...
async def brick_door_height_handler(
    ...
    await message.answer(
        f"🚪 Eshik maydoni: {door_area:.2f} mВІ\n\n"
        "🪟 Endi derazalar sonini kiriting.\n\n"
        "Masalan: 4"
    )
...
async def brick_window_count_handler(
    ...
    if count < 0:
        ...
        await message.answer(
            "❌ Deraza soni manfiy boвЂlishi mumkin emas.\n\n"
            "Agar deraza boвЂlmasa, 0 yuboring."
        )
...
async def brick_window_length_handler(
    ...
    await message.answer(
        f"✅ Deraza uzunligi: {length:g} m\n\n"
        "🪟 Endi deraza boвЂyini kiriting.\n\n"
        "Masalan:\n"
        "1500 mm\n"
        "yoki\n"
        "1.5 m"
...
async def brick_window_height_handler(
    ...
    await message.answer(
        f"🪟 Deraza maydoni: {window_area:.2f} mВІ\n\n"
        "✅ Eshik va derazalar hisoblandi.\n\n"
        "Endi gвЂisht oвЂlchamini tanlang:"
    )
...
async def brick_size_handler(
    ...
    if text == "Boshqa oвЂlcham":
        ...
        await message.answer(
            "📏 GвЂisht oвЂlchamlarini millimetrda kiriting.\n\n"
            "Format:\n"
    ...
    if len(parts) == 3:
        ...
        if l <= 0 or w <= 0 or h <= 0:
            ...
            await message.answer(
                "❌ Barcha oвЂlchamlar 0 dan katta boвЂlishi kerak."
            )
    ...
    await message.answer(
        "❌ GвЂisht oвЂlchami notoвЂgвЂri.\n\n"
...
async def brick_thickness_handler(
    ...
    brick_rates = {
        ...
        "1.5 gвЂisht": {
            "rate": 153,
            "thickness": "380–400 mm"
        },

        "2 gвЂisht": {
            "rate": 255,
            "thickness": "510–520 mm"
        }
    }
    ...
    if text not in brick_rates:
        ...
        await message.answer(
            "❌ Devor qalinligini tugmalardan tanlang."
        )
    ...
    result = (
        ...
        "ℹ️ Hisoblashda eshik va derazalar "
        "maydoni umumiy devor maydonidan ayrildi.\n"

        "ℹ️ GвЂisht sarfi siz belgilagan standart "
        "boвЂyicha hisoblandi.\n"

        "ℹ️ 5% zaxira sinish, kesish va "
...
async def brick_length_handler(
    ...
    if not message.text:
        ...
        await message.answer(
            "❌ Iltimos, uzunlikni son bilan kiriting.\n\n"
            "Masalan: 15"
        )
    ...
    if length <= 0:
        ...
        await message.answer(
            "❌ Devor uzunligi 0 dan katta boвЂlishi kerak.\n\n"
            "Masalan: 10"
        )
    ...
    await message.answer(
        f"✅ Devor uzunligi: {length:g} m\n\n"
        "2️⃣ Endi devor balandligini metrda kiriting.\n\n"
        "Masalan:\n"
        "3\n\n"
        "📌 Faqat son kiriting."
    )
...
async def brick_height_handler(
    ...
    if not message.text:
        ...
        await message.answer(
            "❌ Iltimos, balandlikni son bilan kiriting.\n\n"
            "Masalan: 3"
        )
    ...
    if height <= 0:
        ...
        await message.answer(
            "❌ Devor balandligi 0 dan katta boвЂlishi kerak.\n\n"
            "Masalan: 3"
        )
    ...
    await message.answer(
        f"✅ Devor balandligi: {height:g} m\n\n"
        "3️⃣ GвЂisht oвЂlchamini tanlang:\n\n"
        "📌 Kerakli standart oвЂlchamni belgilang.",
        reply_markup=brick_size_menu
    )


# =========================================================
# 3-QADAM — GвЂISHT OвЂLCHAMI
# =========================================================
...
async def brick_size_handler(
    ...
    if text == "Boshqa oвЂlcham":
        ...
        await message.answer(
            "📏 GвЂishtning oвЂlchamlarini millimetrda "
            "ketma-ket kiriting.\n\n"
            "Format:\n"
            "uzunlik kenglik balandlik\n\n"
            "Masalan:\n"
    ...
    if text in brick_sizes:
        ...
        await message.answer(
            f"✅ GвЂisht oвЂlchami: {brick['name']}\n\n"
            "4️⃣ Devor qalinligini tanlang:",
            reply_markup=thickness_menu
        )
    ...
    if len(parts) == 3:
        ...
        if (
            ...
            await message.answer(
                "❌ GвЂisht oвЂlchamlari 0 dan katta "
                "boвЂlishi kerak."
            )
        ...
        if (
            ...
            await message.answer(
                "❌ GвЂisht oвЂlchami juda katta.\n\n"
...
async def brick_thickness_handler(
    ...
    for key in required_keys:
        ...
        if key not in data:
            ...
            await message.answer(
                "❌ Hisoblash ma’lumotlarida xatolik yuz berdi.\n\n"
    ...
    result = (
        ...
        "ℹ️ Hisoblashda 10 mm qorishma "
        "choklari hisobga olindi.\n"

        "ℹ️ 5% zaxira sinish va chiqindilar "
        "uchun qoвЂshildi."
    )
...
async def gazoblok_length(
    ...
    await message.answer(
        "📏 Endi devor balandligini kiriting.\n\n"
        "Masalan: 3 m"
    )
...
async def gazoblok_height(
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text="🚪 Eshik/deraza bor"),
                KeyboardButton(text="❌ YoвЂq")
            ]
        ],
        resize_keyboard=True
    ...
    await message.answer(
        "🪟 Devorda eshik yoki deraza bormi?",
        reply_markup=keyboard
    )
...
@dp.message(GazoblokCalculator.openings, F.text == "❌ YoвЂq")
...
@dp.message(GazoblokCalculator.openings, F.text == "🚪 Eshik/deraza bor")
...
async def gazoblok_door_width(
    ...
    try:
        ...
        await message.answer("❌ Masalan: 0.9")
        return
...
async def gazoblok_door_height(
    ...
    try:
        ...
        await message.answer("❌ Masalan: 2.1")
        return
...
async def gazoblok_window_count(
    ...
    try:
        ...
        await message.answer("❌ Butun son kiriting. Masalan: 4")
        return
...
async def gazoblok_window_width(
    ...
    try:
        ...
        await message.answer("❌ Masalan: 1.5")
        return
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                ...
                KeyboardButton(text="✏️ Maxsus oвЂlcham")
            ]
        ],
        resize_keyboard=True
    )
...
async def gazoblok_calculate(
    ...
    if text == "✏️ Maxsus oвЂlcham":
        await message.answer(
            "✏️ Maxsus oвЂlchamni quyidagi koвЂrinishda kiriting:\n\n"
            "Masalan:\n"
            "625x250x200"
        )
        return
    ...
    if text not in sizes:
        await message.answer(
            "❌ Iltimos, roвЂyxatdan gazoblok oвЂlchamini tanlang."
        )
        return
    ...
    if useful_area <= 0:
        await message.answer(
            "❌ Eshik va deraza maydoni devor maydonidan katta boвЂlishi mumkin emas."
        )
        await state.clear()
        return
    ...
    await message.answer(
        "🧱 GAZOBLOK HISOB-KITOBI\n\n"
        f"📏 Devor uzunligi: {data['length']:.2f} m\n"
        f"📐 Devor balandligi: {data['height']:.2f} m\n"
        f"🧱 Blok oвЂlchami: {block_length}Г—{block_height}Г—{block_width} mm\n\n"
        f"📐 Devor maydoni: {wall_area:.2f} mВІ\n"
        f"🚪 Eshik/deraza maydoni: {opening_area:.2f} mВІ\n"
        f"📐 Sof devor maydoni: {useful_area:.2f} mВІ\n\n"
        f"🧱 Asosiy miqdor: {basic_quantity:.0f} dona\n"
        f"➕ 5% zaxira: {reserve:.0f} dona\n\n"
        f"✅ KERAKLI GAZOBLOK: {total_quantity:.0f} DONA",
        reply_markup=calculator_menu()
    )
...
async def shlakoblok_show_sizes(
    ...
    await message.answer(
        "🪨 SHLAKOBLOK OвЂLCHAMINI TANLANG:\n\n"
        "ℹ️ KoвЂrsatilgan oвЂlchamlar amaliy "
...
async def shlakoblok_custom_size(
    ...
    await message.answer(
        "✏️ Maxsus shlakoblok oвЂlchamini kiriting.\n\n"
        "Format:\n"
...
async def shlakoblok_calculate_result(
    ...
    await message.answer(
        ...
        f"✅ KERAKLI SHLAKOBLOK:\n"
        f"🪨 {total_quantity_rounded} DONA",
...
# =========================================================
# SHLAKOBLOK — QAYTA HISOBLASH
# =========================================================

@dp.message(F.text == "🔄 Qayta hisoblash")
...
async def beton_start(
    ...
    await message.answer(
        "🏠 BETON KALKULYATORI\n\n"
        "🧱 Beton qalinligi: 400 mm (40 sm)\n\n"
        "📏 Xona uzunligini kiriting.\n\n"
        "Masalan: 4 m",
        reply_markup=keyboard
    )
...
async def beton_width(
    ...
    await message.answer(
        "📏 Endi beton balandligini kiriting.\n\n"
        "Masalan: 2 m"
    )
...
def armatura_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [
                KeyboardButton(text="🏠 Plita"),
                KeyboardButton(text="🏗 Poydevor")
            ],
            [
                KeyboardButton(text="📐 ToвЂsin"),
                KeyboardButton(text="🏛 Ustun")
            ],
            [
                KeyboardButton(text="🧱 Devor"),
...
async def armatura_slab_start(
    ...
    await message.answer(
        "🏠 PLITA ARMATURA HISOBI\n\n"
        "1️⃣ Plita uzunligini kiriting.\n\n"
        "Masalan: 6 m"
    )
...
async def armatura_slab_width(
    ...
    await message.answer(
        "3️⃣ Himoya qatlamini kiriting (mm).\n\n"
        "Masalan: 20"
    )
...
async def armatura_slab_cover(
    ...
    await message.answer(
        "4️⃣ Asosiy armatura diametrini tanlang:",
...
async def armatura_slab_main_diameter(
    ...
    await message.answer(
        "5️⃣ Asosiy armatura qadamini tanlang:",
...
async def armatura_slab_main_spacing(
    ...
    await message.answer(
        "6️⃣ KoвЂndalang armatura diametrini tanlang:",
        reply_markup=diameter_menu()
    )
...
async def armatura_slab_cross_diameter(
    ...
    await message.answer(
        "7️⃣ KoвЂndalang armatura qadamini tanlang:",
        reply_markup=spacing_menu()
    )
...
async def armatura_slab_cross_spacing(
    ...
    if clear_length_mm <= 0 or clear_width_mm <= 0:
        await message.answer(
            "❌ Himoya qatlami plita oвЂlchamiga nisbatan juda katta."
        )
        await state.clear()
        return
    ...
    await message.answer(
        ...
        f"📐 Plita: {length:.2f} Г— {width:.2f} m\n"
        f"🛡 Himoya qatlami: {cover:.0f} mm\n\n"

        "━━━━━━━━━━━━━━\n"
        "🔹 ASOSIY ARMATURA\n"
        "━━━━━━━━━━━━━━\n"

        f"Г{main_d:.0f} — {main_spacing:.0f} mm qadam\n"
        f"📌 Sterjenlar soni: {main_count} dona\n"
        f"📏 Har biri: {main_length_each_m:.2f} m\n"
        f"📐 Jami: {main_total_m:.2f} m\n"
        f"⚖️ OgвЂirligi: {main_weight:.2f} kg\n\n"

        "━━━━━━━━━━━━━━\n"
        "🔹 KOвЂNDALANG ARMATURA\n"
        "━━━━━━━━━━━━━━\n"

        f"Г{cross_d:.0f} — {cross_spacing:.0f} mm qadam\n"
        f"📌 Sterjenlar soni: {cross_count} dona\n"
        f"📏 Har biri: {cross_length_each_m:.2f} m\n"
        f"📐 Jami: {cross_total_m:.2f} m\n"
        f"⚖️ OgвЂirligi: {cross_weight:.2f} kg\n\n"

        "━━━━━━━━━━━━━━\n"
        "📊 YAKUNIY NATIJA\n"
        "━━━━━━━━━━━━━━\n"

        f"📏 Umumiy armatura: {total_m:.2f} m\n"
        f"⚖️ Umumiy ogвЂirlik: {total_weight:.2f} kg\n\n"

        f"➕ 5% zaxira: {reserve_kg:.2f} kg\n\n"

        f"🟢 YAKUNIY: {final_weight:.2f} kg\n"
        f"🟢 Umumiy uzunlik: {final_m:.2f} m\n\n"
...
async def armatura_mesh_start(
    ...
    await message.answer(
        "🕸 ARMATURA SETKASI HISOBI\n\n"
        "1️⃣ Setka uzunligini kiriting.\n\n"
        "Masalan: 6 m"
    )
...
async def mesh_length(
    ...
    if value is None or value <= 0:
        await message.answer("❌ Masalan: 6")
        return
...
async def mesh_width(
    ...
    if value is None or value <= 0:
        await message.answer("❌ Masalan: 4")
        return
...
async def mesh_cover(
    ...
    if value is None or value < 0:
        await message.answer("❌ Masalan: 20")
        return
    ...
    await message.answer(
        "4️⃣ Armatura diametrini tanlang:",
        reply_markup=diameter_menu()
    )
...
async def mesh_spacing(
    ...
    await message.answer(
        ...
        f"📐 OвЂlcham: {length:.2f} Г— {width:.2f} m\n"
        f"🛡 Himoya qatlami: {cover:.0f} mm\n"
        f"🔩 Armatura: Г{diameter:.0f}\n"
        f"📏 Qadam: {spacing:.0f} mm\n\n"

        "━━━━━━━━━━━━━━\n"

        f"↔️ 1-yoвЂnalish: {count_1} dona Г— "
        f"{length_1:.2f} m = {total_1:.2f} m\n"

        f"↕️ 2-yoвЂnalish: {count_2} dona Г— "
        f"{length_2:.2f} m = {total_2:.2f} m\n\n"

        f"📏 Jami: {total_m:.2f} m\n"
        f"⚖️ OgвЂirligi: {total_weight:.2f} kg\n\n"
...
# ============================================================
# 🏗 POYDEVOR ARMATURA KALKULYATORI
# ============================================================

def foundation_type_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🧱 Lenta poydevor")],
            [KeyboardButton(text="⬛ Plita poydevor")],
            [KeyboardButton(text="🟫 Alohida poydevor")],
            [KeyboardButton(text="🏗 Ustunli poydevor")],
            [KeyboardButton(text="🏢 Monolit poydevor")],
            [KeyboardButton(text="⬅️ Ortga")]
        ],
        resize_keyboard=True
    )
...
async def foundation_get_height(
    ...
    await message.answer(
        "4️⃣ Himoya qatlamini kiriting (mm).\n\n"
        "Masalan: 50"
    )
...
async def foundation_get_cover(
    ...
    await message.answer(
        "5️⃣ Asosiy armatura diametrini tanlang:",
...
async def foundation_get_main_diameter(
    ...
    await message.answer(
        "6️⃣ Asosiy boвЂylama armatura sonini kiriting.\n\n"
        "Masalan: 4\n\n"
        "⚠️ Bu loyiha boвЂyicha qabul qilinadigan "
...
async def foundation_get_main_count(
    ...
    await message.answer(
        "7️⃣ Xomut / koвЂndalang armatura diametrini tanlang:",
        reply_markup=foundation_diameter_menu()
    )
...
async def foundation_get_cross_diameter(
    ...
    await message.answer(
        "8️⃣ Xomut qadamini kiriting (mm).\n\n"
        "Masalan: 200"
    )
...
async def foundation_get_cross_spacing(
    ...
    await message.answer(
        "9️⃣ Nechta bir xil poydevor hisoblanadi?\n\n"
        "Masalan: 4"
    )
...
async def foundation_calculate(
    ...
    await message.answer(
        "🏗 Poydevor boвЂyicha yana hisoblash:",
        reply_markup=armatura_menu()
    )
...
@dp.message(F.text == "📞 Admin bilan bogвЂlanish")
